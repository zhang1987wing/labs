...
if __name__ == "__main__":
    file_directory = f"D:\\网赚\\ins视频\\petschanneltv"

    video_groups = video_grouping(file_directory, 2)

    folder_path = f"D:\\网赚\\已处理视频\\petschanneltv"
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)  # 创建文件夹

    for i, group in enumerate(video_groups):
        count = i + 1
        concatenate_videos(group, f"D:\\网赚\\已处理视频\\petschanneltv\\final_output_video_{count}.mp4")
